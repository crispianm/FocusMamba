...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config", required=True, help="Training config or config snapshot."
    )
...
def _build_val_dataset(
    ...
    *,
    ...
    eval_num_frames = int(
        num_frames_override
        or data_cfg.get("val_num_frames", data_cfg.get("num_frames", 8))
    )
    ...
    dataset = TartanAirV2Dataset(
        ...
        seed=int(
            seed_override if seed_override is not None else data_cfg.get("seed", 42)
        ),
...
def _load_checkpoint(
    model: torch.nn.Module, checkpoint_path: str, strict: bool
) -> None:
...
def _select_frames(
    batch: dict[str, Any], input_source: str
) -> tuple[torch.Tensor, str]:
    if input_source == "degraded":
        if "degraded_frames" not in batch:
            raise KeyError(
                "Requested --input-source=degraded but batch has no degraded_frames"
            )
    ...
    if input_source == "clean":
        ...
        raise KeyError(
            "Requested --input-source=clean but batch has no clean/frames tensor"
        )
...
def _aggregate_from_rows(
    rows: list[dict[str, Any]], *, wall_time_sec: float
) -> dict[str, Any]:
    metric_names = (
        "abs_rel",
        "sq_rel",
    ...
    aggregate = {
        ...
        "clips_per_sec": (len(rows) / wall_time_sec)
        if wall_time_sec > 0
        else float("nan"),
    }
...
def evaluate(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        batch_keys = [
            (batch_video_ids[idx], batch_start_frames[idx])
            for idx in range(len(batch_video_ids))
        ]
        ...
        with _autocast_context(device, amp_mode):
            try:
                outputs = model(
                    model_frames, return_gate_stats=wants_gate_stats, **model_kwargs
                )
            except TypeError:
        ...
        gate_rows = (
            _summarize_gate_stats_per_clip(outputs["gate_stats"])
        ...
        for sample_idx in range(int(pred.shape[0])):
            ...
            metrics["fdv"] = float(
                frame_depth_variation(clip_pred.float(), mask=clip_mask.float())
            )
            row = {
        ...
        if log_every > 0 and (batch_idx % log_every == 0 or batch_idx == total_batches):
            elapsed = max(
                previous_wall_time_sec + (time.perf_counter() - wall_start), 1e-6
            )
            print(
...
def main() -> None:
    ...
    if args.resume and existing_rows:
        print(
            f"[resume] loaded {len(existing_rows)} rows from {per_clip_csv}", flush=True
        )
    ...
    if args.resume and aggregate_json.is_file():
        try:
            ...
            previous_wall_time_sec = float(
                existing_report.get("aggregate", {}).get("wall_time_sec", 0.0) or 0.0
            )
        except Exception:
            previous_wall_time_sec = 0.0
    ...
    loader = _build_loader(
        dataset, batch_size=int(args.batch_size), num_workers=num_workers
    )
    ...
    eval_mode = (
        args.mode if args.mode is not None else cfg.get("model", {}).get("mode", None)
    )
    max_depth = float(data_cfg.get("max_depth", 80.0))
    ...
    bootstrap_report = {
        ...
        for idx, metric_name in enumerate(
            ("abs_rel", "delta1", "rmse", "si_log", "fdv")
        )
    }
    ...
    if args.reference_per_clip_csv:
        ...
        paired_bootstrap_json.write_text(
