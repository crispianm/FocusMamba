...
def build_model(cfg: dict) -> "torch.nn.Module":
    ...
    if model_type == "mamba":
        ...
        model = FocusMamba(
            ...
            temporal_mamba_d_state=int(
                model_cfg.get(
                    "temporal_mamba_d_state", model_cfg.get("mamba_d_state", 16)
                )
            ),
            temporal_mamba_d_conv=int(
                model_cfg.get("temporal_mamba_d_conv", model_cfg.get("mamba_d_conv", 4))
            ),
            temporal_mamba_expand=int(
                model_cfg.get("temporal_mamba_expand", model_cfg.get("mamba_expand", 2))
            ),
            ...
            quality_hidden_channels=int(quality_cfg.get("hidden_channels", 16)),
        ...
        common_kwargs = dict(
            ...
            depths=model_cfg.get(
                "encoder_depths", cfg.get("encoder_depths", [2, 2, 4, 2])
            ),
        ...
        transformer_kwargs = {
            **common_kwargs,
            "mlp_ratio": model_cfg.get(
                "transformer_mlp_ratio", cfg.get("transformer_mlp_ratio", 3.15)
            ),
        }
