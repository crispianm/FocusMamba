...
class MultiresConvDecoder(nn.Module):
    ...
    def __init__(
        ...
        # At the highest resolution, i.e. level 0, we apply projection w/ 1x1
        # convolution when the dimensions mismatch. Otherwise we do not do
        # anything, which is the default behavior of monodepth.
        conv0 = (
    ...
    def forward(self, encodings: torch.Tensor) -> torch.Tensor:
        ...
        if num_levels != num_encoders:
            raise ValueError(
                f"Got encoder output levels={num_levels}, "
                f"expected levels={num_encoders}."
            )
