...
from .vit_factory import VIT_CONFIG_DICT, ViTPreset, create_vit
...
def create_backbone_model(
    ...
    Args:
    ----
        preset: A backbone preset to load pre-defined configs.
...
class DepthPro(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        Args:
        ----
            x (torch.Tensor): Input image (B, 3, 1536, 1536), normalised to [-1, 1].

        Returns:
        -------
            The canonical inverse depth map [m^-1] and the optional estimated
            field of view [deg].
    ...
    def infer(
        ...
        Note: if the focal length is given, the estimated value is ignored and the
        provided focal length is used to generate the metric depth values.
