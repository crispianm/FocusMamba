...
#
# Vendored and simplified for da3metric-large:
#   - alt_start=-1 (no camera tokens, no alternating attention)
#   - qknorm_start=-1 (no QK normalization)
#   - rope_start=-1 (no RoPE)
#   - cat_token=False
# Removes: reference_view_selector, OmegaConf, registry dependencies.
...
logger = logging.getLogger("da3.dinov2")
...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], **kwargs):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # All blocks use local attention, no RoPE, no camera tokens.
            x = self.process_attention(x, blk, "local", pos=None)

            if i in blocks_to_take:
                out_x = torch.cat([x, x], dim=-1) if self.cat_token else x
