...
#
# DPT head for dense prediction. Vendored from Depth-Anything-3.
# Replaces addict.Dict with plain dict.
...
class DPT(nn.Module):
    ...
    Returns a plain dict with keys:
      - ``depth``: (B, S, H, W) positive metric depth (exp-activated)
      - ``sky``: (B, S, 1, H/down_ratio, W/down_ratio) if use_sky_head
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Args:
            feats: List of 4 (features, camera_token) tuples from backbone.
            H, W: Original image dimensions.
            patch_start_idx: Starting index of patch tokens.

        Returns:
            dict with 'depth', optionally 'depth_conf' and 'sky'.
        """
