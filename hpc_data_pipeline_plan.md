...
| 5 | **ROI in pipeline** | Removed. ROI is a training-time concern (random crop, user interaction, or saliency), not a data preprocessing artifact. The pipeline produces no `roi` field. |
...
formula that requires absolute distances in metres.
