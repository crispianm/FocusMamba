#!/bin/bash
# =============================================================================
# FocusMamba v2 smoke test (2 epochs) with cached teacher-output distillation.
#
# Usage:
#   sbatch jobs/run_train_focusmamba_v2_smoke_distill.sh
#   sbatch --export=ALL,CONFIG=configs/experiments/<other>.yaml \
#          jobs/run_train_focusmamba_v2_smoke_distill.sh
# =============================================================================

#SBATCH --job-name=fm_v2_smoke
...
CONFIG="${CONFIG:-configs/experiments/tartanair_focusmamba_v2_smoke_distill.yaml}"
EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-}"

echo "========================================"
echo " FocusMamba v2 Smoke Distillation Run"
...
# Optional: set RESET_SMOKE_CHECKPOINTS=1 to force a fresh run.
if [ "${RESET_SMOKE_CHECKPOINTS:-0}" = "1" ] && [ -d "$CHECKPOINT_DIR" ]; then
  echo "RESET_SMOKE_CHECKPOINTS=1 -> removing prior checkpoint dir: $CHECKPOINT_DIR"
